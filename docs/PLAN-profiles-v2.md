...
4. **WS4 — unified full-screen editor** with per-section preflight probes ✅ SHIPPED 2026-07-24. `ProfileEditorModal.tsx` — dimmed modal with name/keywords/roles/remote/salary/companies in one place; sidebar reduced to a list. Per-field live preflight via new `GET /jobs/preflight?keyword=&role_type=` (keyword chips + selected roles show live corpus counts). Companies keep name→match→confirm; the "Know the exact slug" probe panel is removed.

**All four workstreams shipped.** Remaining open item: verify end-to-end that a scrape run ingests remotive/remoteok/themuse jobs (WS3) — needs a cron run or in-browser trigger.
